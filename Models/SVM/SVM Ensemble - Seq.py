...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 20 18:20:17 2023

@author: davidteunissen
This set of Python code creates a functioning ensemble for the SVM
...
The code is set up in the following format:
    ...
    2. We create a function that splits the data in test and train sets and splits the data on relevant variables
    3. We put it all together to create a functioning SVM ensemble
...
#%% 3. We put it all together to create a functioning SVM ensemble
def SVM_ensemble(data, rs, n_models, ycol, xcol, agg_metric_ens, agg_metric_seq):
    ...
    ens_pred_final = pred_aggr(agg_metric_ens_= agg_metric_ens, agg_metric_seq_= agg_metric_seq, 
                               pred = ensemble_results, y_true=ytest)
    
    y_test_eval = ytest.drop_duplicates().sort_values(by='id').reset_index(drop=True)['conversion']
    ...
    results = result_eval(ypred=ens_pred_final, ytrue = y_test_eval)
    ...
    print("Final ensemble results are:")
    print(results)
    return results


#%% Aggregating results
svm_ensemble_seq_res = []
for ens_models in np.arange(5,35,5):
    res = SVM_ensemble(data=df_processed, rs=33, n_models=ens_models, ycol=['id','conversion'],
    ...
    svm_ensemble_seq_res.append(res)

SVM_ensemble_seq_df = pd.concat(svm_ensemble_seq_res, ignore_index=True)

file_path = "/Users/davidteunissen/Desktop/Msc Thesis/Thesis code/Results/SVM_ens_seq_res_5_30_bagged.tsv"
SVM_ensemble_seq_df.to_csv(file_path, sep='\t', index=False)
