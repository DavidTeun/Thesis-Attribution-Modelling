...
#%% 3. We put all of it together and create the final cascade function
def Cascade_SVM(data, x_cols, y_cols,randoms,
            threshold, measure,
            max_models,
    ...
    cascade.append(SVM(X_train=Xtrain1, y_train=ytrain1['conversion']))
    ...
    def pred_aggr(agg_metric_ens_, agg_metric_seq_, pred, y_true):
        
        def ens_aggr(pred_res, metric):
            if metric == 'average':
                aggregate = np.rint(sum(pred_res)/len(pred_res))
        ...
        y_pred_eval = {
            ...
            'y_pred': ens_aggr(pred_res= pred, metric = agg_metric_ens_)
            }
        ...
        def seq_aggr(pred_eval, metric):
            if metric == 'average':
                aggregate = np.rint(pred_eval.groupby(['id'])['y_pred'].mean())
            
            if metric == 'median':
                aggregate = np.rint(pred_eval.groupby(['id'])['y_pred'].median())
            
            if metric == 'min':
                aggregate = np.rint(pred_eval.groupby(['id'])['y_pred'].min())
            
            if metric == 'max':
                aggregate = np.rint(pred_eval.groupby(['id'])['y_pred'].max())
            
            return aggregate
        
        y_pred_final = seq_aggr(y_pred_eval, agg_metric_seq_).reset_index(drop=True)
        
        return y_pred_final
    
    y_val_pred = pred_aggr(agg_metric_ens_= agg_metric_ens, agg_metric_seq_=agg_metric_seq, 
                           pred = prediction, y_true = yval) 
    
    y_val_eval = yval.drop_duplicates().sort_values(by='id')['conversion'].reset_index(drop=True)
    ...
    #First we introduce a function that allows us to update the data based on correclty identified whole 
        ...
        for i in range(1, max_models):
            ...
            cascade.append(SVM(X_train=X_train_casc, y_train=y_train_casc['conversion']))
            
            #Adding a new prediction
            prediction.append(cascade[i].predict(Xval))
            
            #Aggregating predictions to sequence predictions
            y_val_pred = pred_aggr(agg_metric_ens_= agg_metric_ens, agg_metric_seq_=agg_metric_seq, 
                                   pred = prediction, y_true = yval) 
        ...
        y_test_pred = pred_aggr(agg_metric_ens_= agg_metric_ens, agg_metric_seq_=agg_metric_seq, 
                               pred = predictions_test, y_true = ytest) 
...
params_casc.extend(list(product([0.9],np.arange(1,5))))
params_casc = []
...
for params in params_casc:
    ...
    res = Cascade_SVM(data=df_processed, randoms=33,
                                       x_cols = ['cost', 'campaign', 'timestamp','time_since_last_click','cat1', 
                                                 'cat2', 'cat3', 'cat4', 'cat5', 'cat6', 'cat7', 'cat8', 'cat9'], 
                                       y_cols = ['id','conversion'], 
                                       threshold = 0.9, measure = 'AUC', max_models=max_mod, 
                                       agg_metric_ens = 'average', agg_metric_seq = 'average', 
                                       p_discard = p_disc)['Final test results']
    SVM_cascade_res_seq.append(res)

SVM_cascade_res_seq_df = pd.concat(SVM_cascade_res_seq, ignore_index=True)

file_path = "/Users/davidteunissen/Desktop/Msc Thesis/Thesis code/Results/SVM_cas_seq_p_disc_max_mod_bagged.tsv"
SVM_cascade_res_seq_df.to_csv(file_path, sep='\t', index=False)
