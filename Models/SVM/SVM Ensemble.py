...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 20 15:40:21 2023
...
The code is set up in the following format:
    1. We create a function that allows us to subsample the data according to the EasyEnsemble process
       and create a function that splits the data in test and train sets and splits the data on relevant variables
...
#%% 2. We put it all together to create a functioning SVM ensemble
def SVM_ensemble(data, rs, n_models, ycol, xcol):
    ...
    results = result_eval(ypred=ens_test, ytrue = ytest)
...
for ens_models in np.arange(5,35,5):
    res = SVM_ensemble(data= df_processed, rs=33, n_models=ens_models, ycol = 'conversion', 
                        xcol = ['cost', 'campaign', 'timestamp','time_since_last_click','cat1', 
                                'cat2', 'cat3', 'cat4', 'cat5', 'cat6', 'cat7', 'cat8', 'cat9'])
    svm_ensemble_res.append(res)

SVM_ensemble_df = pd.concat(svm_ensemble_res, ignore_index=True)

file_path = "/Users/davidteunissen/Desktop/Msc Thesis/Thesis code/Results/SVM_ens_res_5_30_bagged.tsv"
SVM_ensemble_df.to_csv(file_path, sep='\t', index=False)
