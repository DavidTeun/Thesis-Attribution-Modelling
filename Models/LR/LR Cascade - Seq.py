...
params_casc.extend(list(product([0.6],np.arange(1,10))))
...
for params in params_casc:
    ...
    res = LR_Cascade(data = df_processed,
                         ...
                         y_cols = ['id','conversion'], threshold = 0.9, measure = 'AUC', max_models=max_mod, 
                         agg_metric_ens = 'average', agg_metric_seq = 'average', p_discard = p_disc, 
                         randoms=33)['Final test results']
    lr_cascade_res.append(res)

lr_cascade_df = pd.concat(lr_cascade_res, ignore_index=True)

file_path = "/Users/davidteunissen/Desktop/Msc Thesis/Thesis code/Results/LR_cas_seq_p_disc_n_mod_bagged.tsv"
lr_cascade_df.to_csv(file_path, sep='\t', index=False)
