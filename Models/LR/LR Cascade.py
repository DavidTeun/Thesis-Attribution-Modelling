...
This set of Python code creates a functioning cascade ensemble for the logistic regression
...
def LR_Cascade(data, x_cols, y_cols, 
    ...
    cas_pred = np.rint(cas_prob).astype(int)
    
    #Evaluating results and storing them
    results_models = []
    ...
    if (results_models[0][measure] >= threshold)[0]:
        ...
        y_pred_test = np.rint(y_prob_test).astype(int)
...
#%% Aggregating results - max_models and p_discard
options_pdisc = np.arange(0.1, 0.6, 0.1)
options_maxmodels = np.arange(1,10)
params_casc = list(product(options_pdisc, options_maxmodels))
params_casc.extend(list(product([0.6],np.arange(1,9))))
params_casc = []
params_casc.extend(list(product([0.7],np.arange(1,9))))
params_casc.extend(list(product([0.8],np.arange(1,7))))
params_casc.extend(list(product([0.9],np.arange(1,6))))
params_casc.extend(list(product([1],np.arange(1,5))))
...
for params in params_casc:
    ...
    res = LR_Cascade(data = df_processed,
                            ...
                            p_discard = p_disc)
    lr_cascade_res.append(res['Final test results'])


lr_cascade_res_df = pd.concat(lr_cascade_res, ignore_index=True)
file_path = "/Users/davidteunissen/Desktop/Msc Thesis/Thesis code/Results/LR_cas_res_p_disc_n_mod_bagged.tsv"
lr_cascade_res_df.to_csv(file_path, sep='\t', index=False)
